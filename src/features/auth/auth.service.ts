...
export const loginService = async ({
  ...
  if (!user.isVerified) throw { status: 400, message: "User is not verified" };
...
export const adminVerify2faService = async ({
  email,
  code,
}: {
  email: string;
  ...
  if (!user) throw { status: 400, message: "Invalid or expired 2FA code" };
...
export const verifyEmailService = async ({ token }: { token: string }) => {
  const user = await User.findOne({ verificationToken: token });
  if (!user) throw { status: 404, message: "Invalid or expired token" };
...
export const meService = async ({ userId }: { userId: string }) => {
  const user = await User.findById(userId).select("-password");
  if (!user) throw { status: 404, message: "User not found", data: null };
  return user;
};